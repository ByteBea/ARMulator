...
class History:
    ...
    def signalChange(self, obj, change):
        ...
        """
        print(f"signalChange: {obj.__class__.__name__} → {list(change.keys())}")
        if obj.__class__ not in self.history[-1]:
            self.history[-1][obj.__class__] = {}
        if obj.__class__ not in self.ckpt:
            self.ckpt[obj.__class__] = {}
