...
class Simulator:
    ...
    def __init__(self, memorycontent, assertionTriggers, addr2line, pcInitValue=0):
        ...
        # Initialize componentsù
        #unicorn does not support the PC special behavior, so we need to keep track of the initial value of the PC to simulate it
        self.pcInitValue=pcInitValue
        self.regs = Registers(self.history)
        self.mem = Memory(self.history, memorycontent)
        self.emu = UnicornEmulator()
        self.emu.dichiarazione_var()
        self.emu.bytecode = memorycontent
        self.emu.history = self.history
        self.emu.Reg = self.regs
        self.emu.mem = self.mem
        self.emu.mu = Uc(UC_ARCH_ARM, UC_MODE_ARM)
        self.emu.mappatura_mem()
        self.emu.csprinizale=self.regs.CPSR
        # Initialize decoders
        self.decoders = {
            "BranchOp": BranchOp(),
            "DataOp": DataOp(),
    ...
    def reset(self):
        ...
        self.regs.banks["User"][15].val = self.pcInitValue + self.pcoffset
        self.emu.reset()
        self.fetchAndDecode()
